import React, { useState, useEffect } from 'react';
import { googleAuthService } from '../services/googleAuthService';

const GoogleTest = () => {
  const [clientId, setClientId] = useState<string>('');
  const [status, setStatus] = useState<string>('Ready to test');

  useEffect(() => {
    const id = import.meta.env.VITE_GOOGLE_CLIENT_ID;
    setClientId(id || 'Not configured');
  }, []);

  const handleGoogleLogin = async () => {
    setStatus('Testing Google login...');
    try {
      const result = await googleAuthService.signIn();
      if (result.success) {
        setStatus('✅ Google login successful!');
        console.log('Google login result:', result);
      } else {
        setStatus(`❌ Google login failed: ${result.error}`);
      }
    } catch (error) {
      setStatus(`❌ Error: ${error}`);
    }
  };

  return (
    <div className="min-h-screen flex items-center justify-center bg-gray-100">
      <div className="max-w-md w-full bg-white rounded-lg shadow-md p-6">
        <h1 className="text-2xl font-bold text-center mb-6">Google OAuth Test</h1>
        
        <div className="space-y-4">
          <div>
            <label className="block text-sm font-medium text-gray-700">Client ID Status:</label>
            <p className={`text-sm ${clientId === 'Not configured' ? 'text-red-600' : 'text-green-600'}`}>
              {clientId === 'Not configured' ? '❌ Not configured' : '✅ Configured'}
            </p>
          </div>

          <div>
            <label className="block text-sm font-medium text-gray-700">Test Status:</label>
            <p className="text-sm text-gray-600">{status}</p>
          </div>

          <button
            onClick={handleGoogleLogin}
            disabled={clientId === 'Not configured'}
            className="w-full flex justify-center items-center px-4 py-3 border border-gray-300 rounded-lg shadow-sm bg-white text-sm font-medium text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 disabled:opacity-50 disabled:cursor-not-allowed"
          >
            <svg className="w-5 h-5 mr-3" viewBox="0 0 24 24">
              <path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"/>
              <path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/>
              <path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"/>
              <path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"/>
            </svg>
            Test Google Login
          </button>

          <div className="mt-6 p-4 bg-gray-50 rounded-lg">
            <h3 className="font-medium text-gray-900 mb-2">Configuration Checklist:</h3>
            <ul className="text-sm text-gray-600 space-y-1">
              <li>✅ Google Cloud Console project created</li>
              <li>✅ OAuth 2.0 Client ID created (Web application)</li>
              <li>✅ Authorized origins: http://localhost:3000</li>
              <li>✅ Redirect URIs: http://localhost:3000</li>
              <li>✅ Environment variables configured</li>
            </ul>
          </div>
        </div>
      </div>
    </div>
  );
};

export default GoogleTest;

