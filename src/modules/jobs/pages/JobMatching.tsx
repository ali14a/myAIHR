...
import type { ResumeScan, ResumeListResponse, JobMatchResult } from '@types';
import {
  CheckCircleIcon,
  XCircleIcon,
              ...
              {userResumes.map((resume) => (
                <label key={resume.id} className="flex items-center p-3 border border-gray-200 rounded-lg cursor-pointer hover:bg-gray-50">
                  <input
                    ...
                    name="resume"
                    value={resume.id}
                    checked={selectedResume === parseInt(resume.id)}
        ...
        <div className="space-y-6">
          ...
          <div className="card">
            <div className="flex items-center justify-between mb-6">
              ...
              <div className={`flex items-center px-3 py-1 rounded-full text-sm font-medium ${getMatchColor(matchResult.overall_score || matchResult.match_score)}`}>
                {getMatchIcon(matchResult.overall_score || matchResult.match_score)}
                <span className="ml-1">{matchResult.overall_score || matchResult.match_score}%</span>
              </div>
            </div>
            ...
            <div className="w-full bg-gray-200 rounded-full h-4 mb-4">
              <div
                ...
                style={{ width: `${matchResult.overall_score || matchResult.match_score}%` }}
              ></div>
            </div>
            
            <p className="text-sm text-gray-600">
              {(matchResult.overall_score || matchResult.match_score) >= 80 
                ? "Excellent match! Your resume aligns very well with this job."
                : (matchResult.overall_score || matchResult.match_score) >= 60
              ...
              <div className="space-y-4">
                <div>
                  <div className="flex justify-between text-sm mb-1">
                    ...
                    <span>{Math.round(((matchResult.skills_match?.matched_skills?.length || 0) / ((matchResult.skills_match?.matched_skills?.length || 0) + (matchResult.skills_match?.missing_skills?.length || 0))) * 100)}%</span>
                  </div>
                  <div className="w-full bg-gray-200 rounded-full h-2">
                    <div
                      className="bg-green-600 h-2 rounded-full"
                      style={{ width: `${Math.round(((matchResult.skills_match?.matched_skills?.length || 0) / ((matchResult.skills_match?.matched_skills?.length || 0) + (matchResult.skills_match?.missing_skills?.length || 0))) * 100)}%` }}
                    ></div>
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-1">
                    {matchResult.skills_match?.matched_skills?.map((skill: string, index: number) => (
                      <span
                        key={index}
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-1">
                    {matchResult.skills_match?.missing_skills?.map((skill: string, index: number) => (
                      <span
                        key={index}
              ...
              <div className="space-y-4">
                <div>
                  <div className="flex justify-between text-sm mb-1">
                    ...
                    <span>{Math.round((1 - (matchResult.experience_match?.gaps?.length || 0) / 10) * 100)}%</span>
                  </div>
                  <div className="w-full bg-gray-200 rounded-full h-2">
                    <div
                      className="bg-blue-600 h-2 rounded-full"
                      style={{ width: `${Math.round((1 - (matchResult.experience_match?.gaps?.length || 0) / 10) * 100)}%` }}
                    ></div>
                  </div>
                </div>
                ...
                <div>
                  ...
                  <ul className="space-y-1">
                    {matchResult.experience_match?.gaps?.map((gap: string, index: number) => (
