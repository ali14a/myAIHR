...
class GoogleAuthService {
  ...
  constructor() {
    ...
    if (!this.clientId || this.clientId === 'your-google-client-id') {
      console.warn('Google Client ID not found or not configured. Please set VITE_GOOGLE_CLIENT_ID in your environment variables.');
    }
  }
  ...
  async signIn(): Promise<GoogleAuthResponse> {
    try {
      ...
      if (!this.clientId || this.clientId === 'your-google-client-id') {
        throw new Error('Google Client ID not configured. Please run ./configure-oauth.sh to set up OAuth credentials.');
      }
        ...
        const auth = window.google.accounts.oauth2.initCodeClient({
          ...
          callback: async (response: any) => {
            try {
              ...
              const backendResponse = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:8000'}/api/auth/google`, {
                method: 'POST',
                headers: {
